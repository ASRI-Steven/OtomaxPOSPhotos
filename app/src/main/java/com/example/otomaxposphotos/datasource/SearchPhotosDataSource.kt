...
class SearchPhotosDataSource(val api: PhotosApiService, val keyword: String?, val category: String?) : PagingSource<Int, Data>() {
    override suspend fun load(params: LoadParams<Int>): LoadResult<Int, Data> {
        return try {
            val nextPageNumber = params.key ?: 1
            val response = api.getSearchPhotos(nextPageNumber, keyword, category)
